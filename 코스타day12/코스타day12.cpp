...
/*fgets는 키보드 입력받을때 엔터를 끝까지 기다린다/파일에서 읽어올때 사용/(buf,length,file*)엔터를 끝까지 기다린다
즉 fgets는 엔터를 쳐야만 넘어가는데 scanf는 엔터까지 값에 들어가서?
but 스페이스로 이루어진 긴 문자열 즉 ㅇㅇㅇ ㅇㅇㅇ ㅇㅇㅇ 이런건 못읽음*/
int getint()
{
...
void Stringparse()
{
	while (1)
	{
		...
		//scanf("%d", &k); 최종 keyborad buffer의 [Enter]키 처리 필요
		fgets(b, 20, stdin);
		...
		else if (*b == '4')
		{
			...
			printf("두 번째 정수를 입력하세요 : ");
			y1 = getdouble();
			printf("두 개의 실수 %.2f와 %.2f 중 작은 수는 %.2f입니다.\n\n\n ", x1, y1, MIN(x1, y1));
		}
		else if (*b == '0') break;
	}
}
